...
function ordersDiscounted(arr) {
    for (let i = 0; i < arr.length; i++) {

        const date = Number(arr[i].timestamp);

        if (date >= 1592352000 && date <= 1594944000) {
            orders[i].price = `${Number(orders[i].price) / 1.2}`;
            orders2.push(arr[i]);
        }
    }
    console.log(orders2);
